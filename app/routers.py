...
async def broadcast_admin_message( request: Request, data = Body()):
    ...
    if target == 'all':
        ...
        await request.app.manager.send_personal_message('{"command":"message","text":"'+message+'"}', target)
    return {'details':'done'}
#-------------------------------------------------------------------------------------------
...
async def service_list(request: Request):
    cmd_to_execute = f'docker service ls --format "{\"ID\":{{.ID}},\"Name\":{{.Name}},\"Replicas\":\"{{.Replicas}}\"}"'
    ssh_stdin, ssh_stdout, ssh_stderr = request.app.ssh_client.exec_command(cmd_to_execute)
